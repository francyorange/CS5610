...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <br /><br />
            {/* update completed status ID=1 using checkbox*/}
            <a href={`${API}/1/completed/${todo.completed}`} className="btn btn-primary float-end">
                Complete Todo ID = 1</a>
            <input type="checkbox" className="form-check-input" defaultChecked={todo.completed}
                onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} />
            <br /><br />
            {/* update description of ID=1 using text input field*/}
            <a href={`${API}/1/description/${todo.description}`} className="btn btn-primary float-end">
                Describe Todo ID = 1</a>
            <input defaultValue={todo.description} className="form-control w-50"
